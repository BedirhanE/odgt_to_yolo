...
annotations_path = "C:/Users/bedir/OneDrive/Masaüstü/odgt_to_yolo/data/annotation_train.odgt"
...
def convert(size, box, img_id):
    if box[0] > size[0] or box[2] > size[0] or box[1] > size[1] or box[3] > size[
        1]:  ###wrong labelling, label coordinate is bigger than or overside the image size
        ...
        if os.path.exists(image_path):
            ...
            for iid, infoBody in enumerate(data["gtboxes"]):
                if infoBody['tag'] != 'mask':
                    ...
                    print("[Left over] {}/{} , {}: head:{} full:{} visible:{}".format(total_lines - lineID,
                                                                                      total_iid - iid, img_id, \
                                                                                      hbox, fbox,
                                                                                      vbox))  # end of one label object loop
